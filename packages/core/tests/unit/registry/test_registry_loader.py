...
def test_registry_accepts_cover_params_list(tmp_path: Path) -> None:
    providers_dir = tmp_path / "providers"
    provider_dir = providers_dir / "demo"
    routes_dir = provider_dir / "routes"
    models_dir = provider_dir / "models"
    ...
    (models_dir / "demo-model.json5").write_text('{ routes: ["chat"] }\n', encoding="utf-8")

    suites = load_SuiteSpecs(providers_dir)

    test_case = next(t for t in suites[0].tests if t.name == "tool_combo")
    assert [p.name for p in test_case.cover_params] == ["tools", "tool_choice"]
